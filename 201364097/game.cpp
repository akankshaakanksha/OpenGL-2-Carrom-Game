...
class Striker : public Coin   {
	public:
		int mass,active,shoot_status,mouseactive;
...
void striker_friction(void)
{
	...
	if(striker.active==1)
	{
		...
		striker.vel_x=(striker.vel_x/vel_striker)*(vel_striker-0.0005f);
		striker.vel_y=(striker.vel_y/vel_striker)*(vel_striker-0.0005f);
	...
	if(coin[i].active==1)
		{
			...
			coin[i].vel_x=(coin[i].vel_x/coin[i].vel)*(coin[i].vel-0.0001f);
			coin[i].vel_y=(coin[i].vel_y/coin[i].vel)*(coin[i].vel-0.0001f);
...
void drawmText(char* text,float x, float y , float z)
{
	char *c;
	glPushMatrix();
	glTranslatef(x,y,z);
...
void drawScene() {
    ...
    if(score<0)
    {
    	...
    	drawText("Game Over!! You Lost!!");
    }

    else {sprintf(scoreh,"Score:%d",score); 
...
void myMouse(int b, int s, int x, int y) {      // mouse click callback
	//printf("%d dshbvh %d",x,y);
	if (s == GLUT_DOWN) {
		if (b == GLUT_RIGHT_BUTTON) {
			float pos_x=striker.pos_x;
			if(pos_x<1.45f && pos_x>-1.45f)
				pos_x=x;
			striker.mouseactive=1;


		}	
		if (b == GLUT_LEFT_BUTTON && striker.vel_x == 0 && striker.vel_y == 0 && striker.pos_y == -1.42f ) {
			striker.mouseactive = -1;
	...
	if( s == GLUT_UP)
	{
		if( striker.mouseactive == -1 )
		{
			
			
				
			float angle=atan(((float)(x-455)/100.0f-striker.pos_x )/ ((float)(y-256.5)/100.0f-striker.pos_y));	
			
			striker.vel_x=sin(angle)*0.01;
			printf("%f\n",striker.vel_x);
			striker.vel_y=cos(angle)*0.01;
			printf("%f\n",striker.vel_y);
			striker.active=1;
